...
import com.android.fusion.example.network.ApiClient;
import com.android.fusion.example.network.ApiService;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseInstanceId.getInstance().getInstanceId().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult() != null) {
                ...
                Log.e("Meating_type" , meetingType);

                if (meetingType != null) {
                    if (getIntent().getBooleanExtra("isMultiple", false)) {
                        Type type = new TypeToken<ArrayList<User>>(){}.getType();
                        ArrayList<User> receivers = new Gson().fromJson(getIntent().getStringExtra("selectedUsers"), type);
                        Log.e("receivers" , receivers.toString());
                        if (receivers != null) {
...
//                        initiateMeeting(meetingType, user.token, null);
    ...
    private void initiateMeeting(String meetingType, String receiverToken, ArrayList<User> receivers) {
        ...
        Log.e("user1" , meetingType);
        Log.e("user2" , receiverToken);
...
//            data.put(Constants.KEY_LAST_NAME, preferenceManager.getString(Constants.KEY_LAST_NAME));
    ...
    protected void onStart() {
        super.onStart();
        LocalBroadcastManager.getInstance(OutgoingInvitationActivity.this).registerReceiver(
    ...
    protected void onStop() {
        super.onStop();
        LocalBroadcastManager.getInstance(OutgoingInvitationActivity.this).unregisterReceiver(
                invitationResponseReceiver
        );
    }
}